...
internal fun Route.issueRoutes(repository: IssueRepository) {
    ...
    get<Projects.Id.Issues.Distance> {
        val qp = call.request.queryParameters
        val targetLocation = Location(qp["latitude"]!!.toDouble(), qp["longitude"]!!.toDouble())
        val maxDistance = qp["maxDistance"]?.toInt() ?: 0
        val issues = repository.filterIssuesByDistance(it.parent.parent.projectId, targetLocation, maxDistance)
        call.respond(issues)
    }
