...
/**
 * Represents all server-side operations for defining and manipulating stored [Comment] data.
 */
internal class CommentRepository : BaseRepository {
    /**
     * Inserts a new [Comment] record.
     *
     * @return The newly created [Comment] with its database-generated id and timestamps.
     */
    suspend fun addComment(comment: Comment): Comment = query {
     ...
     */
    suspend fun editComment(comment: Comment): Comment = query {
        val returnStmt = Comments
            .updateReturning(
     ...
     */
    suspend fun deleteComment(id: Long): Boolean = query {
        val returnStmt = Comments
            .deleteReturning(
